...
public class GetTodaysGamesLogic {
    public static List<GameData> getGamesFromDate(String date, LambdaLogger logger, Gson gson) {
        ...
        GamesFromDateResponse todaysResponse = gson.fromJson(gamesFromTodayResponse.get().body(), GamesFromDateResponse.class);
        ...
        GamesFromDateResponse yesterdaysResponse = gson.fromJson(gamesFromTodayResponse.get().body(), GamesFromDateResponse.class);

        List<Game> todaysGames = todaysResponse.getApi().getGames().stream().filter(GameUtils::isGameFinished)
                .filter(GameUtils::gameStartedBeforeFive)
                .collect(Collectors.toList());

        List<Game> yesterdaysGames = todaysResponse.getApi().getGames().stream().filter(GameUtils::isGameFinished)
        ...
        List<GameData> allGames = Stream.concat(todaysGames.stream(), yesterdaysGames.stream())
                ...
                .collect(Collectors.toList());
        logger.log("games found: " + allGames.stream().map(GameData::getId).collect(Collectors.toList()));
